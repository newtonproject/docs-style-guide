...
* Mark the file as copyright protected

* The copyright notice should include following:
...
* Use [http://jshint.com/ JSHint] for code analysis (error and style checking)

## Folder Naming ##
* Rules for naming folder: [ModuleName]-[ChildFunction].js
* If the module is a directory, the rule is: [ChildFunction].js

## Code Compression ##
* All Javascript files include two versions, one is development environment version (e.g. foo.js), and the other is production environment version (e.g. foo.min.js).
* Use [https://developers.google.com/closure/compiler/closure] to compress the code
...
* Commit Javascript file, and don't forget to compress the code. 


## Unit Testing ##
Test folder should locate under root directory of Django project
### Test Case ###
Use [https://qunitjs.com/ Qunit] to do Javascript Unit Testing
...
```
Execute python -m http.server (or python -m SimpleHTTPServer on Python 2) from the root directory (not from inside js_tests).
```

Use browser to open http://localhost:8000/js_tests/tests.html .
